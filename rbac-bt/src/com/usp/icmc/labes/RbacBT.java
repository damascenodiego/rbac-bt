...
//		if(cmd.hasOption(SSOD))	types.add(MutantType.SSoD_REPLACE);
	...
	private static void runTest(CommandLine cmd, File output) {
		Chronometer chron = new Chronometer();
		chron.start();
	...
	private static void setupCliOptions() {
		...
		Option runTestOption = new Option(RUNTEST_SHORT_PARAMETER,RUNTEST_PARAMETER, true, 		"Runs a conformance testing procedure of an FSM/RBAC specification against a set of RBAC policies using an .testcnf file.\n");
